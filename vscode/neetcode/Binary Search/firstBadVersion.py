...
# ------------------------------------------------------------
"""
時間: O(log n)  ← 毎回半分に絞る二分探索
空間: O(1)

ポイント:
1. これは "条件を満たす最小の値" を探す典型的な二分探索パターン。
   `bad` 以降は全て True、それより前は全て False という単調性を
   利用して二分探索が成立する。

2. 等号の扱い:
   - 通常の "値を探す" 二分探索: while left <= right, return -1 if not found
   - "境界を探す" 二分探索:      while left <  right, return left
   後者を使うのは、left == right に達した瞬間にそれが答えになるから。
   今回のような "first bad" 問題では後者がきれい。

3. high = mid (mid + 1 ではない) なのも重要。
   mid 自身が答えの候補なので、捨てずに範囲に残す必要がある。

4. APIコール回数を最小化したいときの定番アプローチ。線形探索だと O(n) だが
   二分探索なら n=10^9 でも約 30 回の API コールで済む。
"""
