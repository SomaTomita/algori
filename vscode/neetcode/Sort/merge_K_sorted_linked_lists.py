...
merged = MergeSolution().mergeKLists(
    [
        ListNode(1, ListNode(4, ListNode(5))),
        ListNode(1, ListNode(3, ListNode(4))),
        ListNode(2, ListNode(6)),
...
print(to_list(merged))  # [1, 1, 2, 3, 3, 4, 4, 5, 6] と書いてあるが
                         # 実際の入力は [1,4,5],[1,3,4],[2,6] なので [1,1,2,3,4,4,5,6]
...
トーナメント (上の解法):
  時間: O(N log k)
    - ペアでマージしていくので "ラウンド" は log k 回
    - 各ラウンドで全 N ノードに 1 回触れる
  空間: O(1)  ← 既存ノードを繋ぎ替えるだけ (再帰せず反復で書いている)

その他のアプローチ:
  - 全部を 1 個ずつ順にマージ: O(N * k)  (各ノードが最大 k 回コピーされる)
  - 最小ヒープ (heapq): O(N log k)        (毎回 k 個の先頭から最小を取り出す)
  - 全要素を 1 つの配列に集めてソート: O(N log N)

ポイント:
1. 「2 つのリストをマージする」関数 (mergeList) が再利用できれば、
   分割統治の発想で k 個のマージは log k ラウンドにできる。

2. for i in range(0, len(lists), 2) はペア取り出しの定番。
   奇数本なら最後 1 本は l2 = None でそのまま残す
   (mergeList(l1, None) は l1 をそのまま返す)。

3. ヒープ解と漸近計算量は同じだが、定数倍はトーナメントの方が小さいことが多い
   (heapq の比較が連結リストの比較関数で重くなりがち)。
"""
