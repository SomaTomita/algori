...
# ------------------------------------------------------------
"""
解法1 (1 キューで pop 時に回す版, MyStack):
  push    : O(1)
  pop     : O(n)  ← 末尾以外を全部キューの後ろに回してから popleft
  top     : O(1)  ← q[-1] でアクセス
  empty   : O(1)
  空間    : O(n)

解法2 (2 キューで push 時に回す版, MyStack_2queues):
  push    : O(n)  ← 既存要素を全て後ろに回すので毎回 n 操作
  pop     : O(1)
  top     : O(1)
  empty   : O(1)
  空間    : O(n)
...
ポイント:
1. キュー (FIFO) でスタック (LIFO) を作るには「push 時に並び替える」か
   「pop 時に並び替える」かの 2 択。どちらかを必ず O(n) にする必要がある。

2. どちらが良いかは "push と pop どちらが多いか" による:
   - pop/top が多い → 解法2 (push を遅くする)
   - push が多い   → 解法1 (pop を遅くする)

3. 解法1 で q[-1] が O(1) なのは Python の deque がリングバッファ実装のため。
   普通のキュー (= 配列を片側からだけ取り出す) では top も O(n) になる。
   厳密なキューAPI制約下では解法2 (2 キュー) が正攻法。

4. 1 キュー版でフォローアップに答えるなら、push 時に並び替える書き方も可:
     def push(self, x):
         self.q.append(x)
         for _ in range(len(self.q) - 1):
             self.q.append(self.q.popleft())
   こうすると push が O(n)、pop/top が O(1) になる。
"""
