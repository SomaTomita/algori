"""
Koko Eating Bananas
You are given an integer array piles where piles[i] is the number of bananas in the ith pile.
You are also given an integer h, which represents the number of hours you have to eat all the bananas.

You may decide your bananas-per-hour eating rate of k.
Each hour, you may choose a pile of bananas and eats k bananas from that pile.
If the pile has less than k bananas, you may finish eating the pile but you can not eat from another pile in the same hour.

Return the minimum integer k such that you can eat all the bananas within h hours.

Example 1:
Input: piles = [1,4,3,2], h = 9
Output: 2
Explanation: With an eating rate of 2, you can eat the bananas in 6 hours.
 With an eating rate of 1, you would need 10 hours to eat all the bananas (which exceeds h=9), thus the minimum eating rate is 2.

Example 2:
Input: piles = [25,10,23,4], h = 4
Output: 25

Constraints:
1 <= piles.length <= 1,000
piles.length <= h <= 1,000,000
1 <= piles[i] <= 1,000,000,000
"""

from typing import List
import math


class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        l, r = 1, max(piles)
        res = r

        while l <= r:
            k = (l + r) // 2

            totalTime = 0
            for p in piles:
                totalTime += math.ceil(float(p) / k)
            if totalTime <= h:
                res = k
                r = k - 1
            else:
                l = k + 1
        return res


print(Solution().minEatingSpeed([1, 4, 3, 2], 9))

"""
Binary Search の流れ: piles = [1,4,3,2], h = 9

各パイル: [1, 4, 3, 2]
制限時間: 9時間
目標: 条件を満たす最小のk

========================================

初期状態: l=1, r=4, res=4
探索範囲: k ∈ [1, 2, 3, 4]

========================================

Iteration 1: l=1, r=4
条件チェック: l(1) <= r(4) → True

k計算: k = (1 + 4) // 2 = 2

totalTime計算 (k=2で試す):
- pile[0]=1: ceil(1/2) = 1時間
- pile[1]=4: ceil(4/2) = 2時間
- pile[2]=3: ceil(3/2) = 2時間
- pile[3]=2: ceil(2/2) = 1時間
totalTime = 1+2+2+1 = 6時間

判定: totalTime(6) <= h(9) ? → True (条件を満たす)
処理: 
- res = k = 2 (答えの候補を更新)
- r = k - 1 = 2 - 1 = 1 (より小さいkを探す)

新探索範囲: k ∈ [1]

========================================

Iteration 2: l=1, r=1
条件チェック: l(1) <= r(1) → True

k計算: k = (1 + 1) // 2 = 1

totalTime計算 (k=1で試す):
- pile[0]=1: ceil(1/1) = 1時間
- pile[1]=4: ceil(4/1) = 4時間
- pile[2]=3: ceil(3/1) = 3時間
- pile[3]=2: ceil(2/1) = 2時間
totalTime = 1+4+3+2 = 10時間

判定: totalTime(10) <= h(9) ? → False (時間オーバー)
処理: l = k + 1 = 1 + 1 = 2

新探索範囲: 空 (l > r)

========================================

Iteration 3: l=2, r=1
条件チェック: l(2) <= r(1) → False
while ループ終了

結果: return res = 2 ✓

========================================
"""

print(Solution().minEatingSpeed([25, 10, 23, 4], 4))

"""
Binary Search の流れ: piles = [25,10,23,4], h = 4

各パイル: [25, 10, 23, 4]
制限時間: 4時間
目標: 条件を満たす最小のk

========================================

初期状態: l=1, r=25, res=25
探索範囲: k ∈ [1, 2, ..., 25]

========================================

Iteration 1: l=1, r=25
k = (1 + 25) // 2 = 13

totalTime計算 (k=13):
- ceil(25/13) = 2時間
- ceil(10/13) = 1時間
- ceil(23/13) = 2時間
- ceil(4/13) = 1時間
totalTime = 2+1+2+1 = 6時間

判定: 6 <= 4 ? → False
処理: l = 13 + 1 = 14

新探索範囲: k ∈ [14, ..., 25]

========================================

Iteration 2: l=14, r=25
k = (14 + 25) // 2 = 19

totalTime計算 (k=19):
- ceil(25/19) = 2時間
- ceil(10/19) = 1時間
- ceil(23/19) = 2時間
- ceil(4/19) = 1時間
totalTime = 2+1+2+1 = 6時間

判定: 6 <= 4 ? → False
処理: l = 19 + 1 = 20

新探索範囲: k ∈ [20, ..., 25]

========================================

Iteration 3: l=20, r=25
k = (20 + 25) // 2 = 22

totalTime計算 (k=22):
- ceil(25/22) = 2時間
- ceil(10/22) = 1時間
- ceil(23/22) = 2時間
- ceil(4/22) = 1時間
totalTime = 2+1+2+1 = 6時間

判定: 6 <= 4 ? → False
処理: l = 22 + 1 = 23

新探索範囲: k ∈ [23, 24, 25]

========================================

Iteration 4: l=23, r=25
k = (23 + 25) // 2 = 24

totalTime計算 (k=24):
- ceil(25/24) = 2時間
- ceil(10/24) = 1時間
- ceil(23/24) = 1時間
- ceil(4/24) = 1時間
totalTime = 2+1+1+1 = 5時間

判定: 5 <= 4 ? → False
処理: l = 24 + 1 = 25

新探索範囲: k ∈ [25]

========================================

Iteration 5: l=25, r=25
k = (25 + 25) // 2 = 25

totalTime計算 (k=25):
- ceil(25/25) = 1時間
- ceil(10/25) = 1時間
- ceil(23/25) = 1時間
- ceil(4/25) = 1時間
totalTime = 1+1+1+1 = 4時間

判定: 4 <= 4 ? → True (条件を満たす)
処理:
- res = k = 25
- r = 25 - 1 = 24

新探索範囲: 空

========================================

Iteration 6: l=25, r=24
条件チェック: l(25) <= r(24) → False
while ループ終了

結果: return res = 25 ✓
========================================
"""


# ------------------------------------------------------------
# ------------------------------------------------------------


# 総当たり解法（Brute Force）
class BruteForceSolution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        # k=1から始めて、条件を満たす最小のkを探す
        for k in range(1, max(piles) + 1):
            totalTime = 0
            for p in piles:
                totalTime += math.ceil(p / k)

            if totalTime <= h:
                return k  # 最初に見つかったkが最小値

        return max(piles)  # 理論上ここには到達しない


print(BruteForceSolution().minEatingSpeed([1, 4, 3, 2], 9))

"""
総当たり解法の流れ: piles = [1, 4, 3, 2], h = 9

目標: h時間以内に全てのバナナを食べられる最小のk

========================================

k=1 を試す:
pile[0]=1: ceil(1/1) = 1時間
pile[1]=4: ceil(4/1) = 4時間  
pile[2]=3: ceil(3/1) = 3時間
pile[3]=2: ceil(2/1) = 2時間
totalTime = 1 + 4 + 3 + 2 = 10時間

判定: 10 <= 9 → False
k=1では時間オーバー、次を試す

========================================

k=2 を試す:
pile[0]=1: ceil(1/2) = ceil(0.5) = 1時間
pile[1]=4: ceil(4/2) = ceil(2.0) = 2時間
pile[2]=3: ceil(3/2) = ceil(1.5) = 2時間
pile[3]=2: ceil(2/2) = ceil(1.0) = 1時間
totalTime = 1 + 2 + 2 + 1 = 6時間

判定: 6 <= 9 → True ✓
k=2が最初に条件を満たす

結果: return 2
========================================
"""


print(BruteForceSolution().minEatingSpeed([25, 10, 23, 4], 4))

"""
総当たり解法の流れ: piles = [25, 10, 23, 4], h = 4

========================================

k=1 を試す:
pile[0]=25: ceil(25/1) = 25時間
pile[1]=10: ceil(10/1) = 10時間
pile[2]=23: ceil(23/1) = 23時間
pile[3]=4: ceil(4/1) = 4時間
totalTime = 25 + 10 + 23 + 4 = 62時間

判定: 62 <= 4 → False

========================================

k=2 を試す:
totalTime = ceil(25/2) + ceil(10/2) + ceil(23/2) + ceil(4/2)
          = 13 + 5 + 12 + 2 = 32時間

判定: 32 <= 4 → False

========================================

k=3 を試す:
totalTime = ceil(25/3) + ceil(10/3) + ceil(23/3) + ceil(4/3)
          = 9 + 4 + 8 + 2 = 23時間

判定: 23 <= 4 → False

========================================

... (中略: k=4~24まで全てFalse) ...

========================================

k=24 を試す:
totalTime = ceil(25/24) + ceil(10/24) + ceil(23/24) + ceil(4/24)
         = 2 + 1 + 1 + 1 = 5時間

判定: 5 <= 4 → False

========================================

k=25 を試す:
totalTime = ceil(25/25) + ceil(10/25) + ceil(23/25) + ceil(4/25)
         = 1 + 1 + 1 + 1 = 4時間

判定: 4 <= 4 → True ✓

結果: return 25
"""


# ------------------------------------------------------------
# 計算量まとめ
# ------------------------------------------------------------
"""
n = len(piles), M = max(piles)

二分探索解 (Solution):
  時間: O(n * log M)
    - 探索範囲は [1, M] なので二分探索は O(log M) 回
    - 各 k の判定 (totalTime 計算) は piles 全走査で O(n)
  空間: O(1)

総当たり解 (BruteForceSolution):
  時間: O(n * M)
    - k = 1, 2, ..., M を全部試す
  空間: O(1)

なぜ二分探索が成立するか:
  k を大きくするほど totalTime は単調に減る (= 速く食べれば早く終わる)。
  ある k* を境に "k < k* なら間に合わない / k >= k* なら間に合う" という
  単調性があるので、二分探索で境界 k* を絞り込める。

ポイント:
1. これは "答えそのもの" を二分探索する問題 (Parametric Search)。
   配列の要素ではなく、答えの候補値の範囲を二分探索する点が特徴。
   "条件を満たす最小値" 系の問題で頻出のパターン。

2. 範囲は [1, max(piles)]:
   - 下限 1: 1 本/時より遅くは食べられない (整数制約)
   - 上限 max(piles): どんな大きいパイルでも 1 時間で 1 山片付くので、
     これより速くする意味がない (h >= len(piles) が制約で保証される)

3. ceil(p / k) は math.ceil(p / k) でも書けるが、
   整数演算で (p + k - 1) // k と書く方が浮動小数点誤差がなく安全。
"""
